...
```{r bizind, echo=FALSE, warning=FALSE, fig.height=6}
year <- seq(2010,2015)
kazscore <- c(57.07,60.24,63.18,63.12,63.59,64.59)
uzbscore <- c(40.83,41.82,44.83,47.69,50.02,54.26)
kazbiz <- cbind(year,kazscore,"Kazakhstan")
uzbbiz <- cbind(year,uzbscore,"Uzbekistan")
bizind <- rbind(kazbiz,uzbbiz)
bizind <- as.data.frame(bizind)
names(bizind) <- c("Year", "Score", "Country")
bizind$Score <- as.numeric(as.character(bizind$Score))
bizind$Year <- as.numeric(as.character(bizind$Year))
ggplot(data = bizind, aes(x = Year)) + 
        geom_bar(aes(y = Score), stat = "identity", position = "dodge") +
        facet_grid(. ~ Country) +
        ggtitle("Improving Business Climate")
altgph3 <- ggplot(data = bizind, aes(x = Year,y = Score, fill = Country)) + 
        geom_bar(stat = "identity", position = "dodge") + 
        ggtitle("Business Institutional Measures")
```
